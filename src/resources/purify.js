 ...
 */
   ...
   * of importing it into a new Document and returning a sanitized copy */
    ...
    const FORBID_CONTENTS = addToSet({}, ['audio', 'colgroup', 'head', 'math', 'script', 'style', 'template', 'thead', 'svg', 'video'])
    ...
    const DEFAULT_URI_SAFE_ATTRIBUTES = addToSet({}, ['alt', 'class', 'for', 'id', 'label', 'name', 'pattern', 'placeholder', 'summary', 'title', 'value', 'style', 'xmlns'])
   ...
   */
    ...
    if (DOMPurify.isSupported) {
      (function () {
        try {
          const doc = _initDocument('<svg><p><textarea><img src="</textarea><img src=x abc=1//">')
   ...
   */
    ...
    const _sanitizeAttributes = function _sanitizeAttributes (currentNode) {
      ...
      let attributes = currentNode.attributes
      ...
      while (l--) {
        attr = attributes[l]
        const _attr = attr
        const name = _attr.name
        const namespaceURI = _attr.namespaceURI
