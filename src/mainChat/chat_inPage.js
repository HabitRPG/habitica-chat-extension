...
function translateMessageText (messageID) {
  ...
  var messageElement = document.getElementById(messageID);
  var chatMarkdown = messageElement.getAttribute('data-markdown');
  var message = removeMentions(chatMarkdown);
  var url = `https://translate.google.com/?sl=auto&tl=en&text=${encodeURIComponent(message)}&op=translate`;
  window.open(url);
  }
    ...
    for (var key in chatData) {
      if (chatData.hasOwnProperty(key) && chatData[key]['text'] !== null ) {
        if (typeof chatData[key]['user'] !== 'undefined' || chatData[key]['uuid'] == "system") {
          ...
          var userLabel = '<span onClick="mention(\''+DOMPurify.sanitize(chatBoxId)+'\',\''+DOMPurify.sanitize(chatData[key]['username'])+'\')">' + DOMPurify.sanitize('<span class="label label-default '+DOMPurify.sanitize(contributorLabel)+'">&nbsp;'+DOMPurify.sanitize(chatData[key]['user'])+'&nbsp;' + DOMPurify.sanitize(userSymbol ) +'</span>&nbsp;&nbsp;<span class="label label-username">@' + DOMPurify.sanitize(chatData[key]['username'] ) + '</span></span>');
